...
async function harvestweedMsg(channel: TextChannel, msg: Message<boolean>) {
    ...
    let db: { weed: number; weedBags: any; busyHarvesting: boolean; save: () => void; }
    ...
    if (db.busyHarvesting === true) {
        return "You're already harvesting. Don't get too excited."
    }

    db.busyHarvesting = true
    db.save()
